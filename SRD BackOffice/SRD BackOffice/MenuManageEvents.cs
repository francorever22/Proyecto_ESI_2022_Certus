namespace SRD_BackOffice
{
    public partial class MenuManageEvents : Form
    {
        ...
        {
            int x = 0;
            var eventos = Logica.GetEventos(1, null);
            int count = eventos.Count();
            foreach (var evento in eventos)
            {
                var fases = Logica.GetFases(2, ""+evento.IdEvento);
                if (x >= count - 15)
                {
        ...
        private void Delete_Click(object sender, EventArgs e, int id, Panel p)
        {
            ...
            if (dialogResult1 == DialogResult.Yes)
            {
                ...
                if (dialogResult2 == DialogResult.Yes)
                {
                    try
                    {
                        Logica.Delete("Eventos", "IdEvento", id + "");
                        p.Dispose();
        ...
        private void btnBuscar_Click(object sender, EventArgs e, string busqueda)
        {
            ...
            var eventos = Logica.GetEventos(2, busqueda);
            int count = 0;
